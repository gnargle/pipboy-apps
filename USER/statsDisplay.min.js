...
function buildScreen(a){if(null==lastReload){displayedPerks=[];try{var b=require("fs").readdirSync(normalizeDir(a))}catch{require("fs").mkdir(normalizeDir(a)),b=require("fs").readdirSync(normalizeDir(a))}b=b.filter(e=>"."!==e&&".."!==e);isModernVersion&&(b=b.filter(e=>{try{return!require("fs").statSync(normalizeDir(a)+"/"+e).isDirectory}catch(g){return console.log("Skipping bad file or folder:",e),!1}}));loadedListMax=b.length;if(0==b.length){drawEmptyScreen();return}let f=Math.min(entryListDisplayMax,
loadedListMax);for(var c=0;entrySelected>=f;)c++,f=Math.min(entryListDisplayMax*(c+1),loadedListMax);for(c*=entryListDisplayMax;c<f;c++){let e=b[c];const g=normalizeDir(a)+"/"+e;let k;try{k=require("fs").readFileSync(g)}catch(l){console.log("Skipping unreadable file:",g,l);continue}let h;try{h=JSON.parse(k)}catch(l){console.log("Invalid JSON in file:",g);continue}screenSelected!=perkScreen?displayedPerks.push({title:h.title,filename:e,entryNum:c,points:h.points}):displayedPerks.push({title:h.title,
filename:e,entryNum:c})}lastReload=entrySelected}if(null==currentPerk){b=displayedPerks[entrySelected%6].filename;b=normalizeDir(a)+"/"+b;try{var d=require("fs").readFileSync(b);currentPerk=JSON.parse(d)}catch(f){console.log("Error loading currentPerk file:",b,f),currentPerk=null}}for(d=0;d<displayedPerks.length;d++)b=displayedPerks[d],b.entryNum==entrySelected&&(drawEntry(currentPerk),drawSelectedEntryOutline(d),configMode||screenSelected==perkScreen||(pointsOfSelected=b.points)),drawEntryTitle(b.title,
d,b.entryNum==entrySelected),screenSelected!=perkScreen&&drawEntryPoints(b.points,d,b.entryNum==entrySelected)}
...
function generatePerksConfigLists(){try{var a=require("fs").readdirSync(normalizeDir(enabledPerkFolder))}catch{require("fs").mkdir(enabledPerkFolder),a=require("fs").readdirSync(normalizeDir(enabledPerkFolder))}for(file of a)enabledPerks.push(file);a=require("fs").readdirSync(normalizeDir(allPerkFolder));loadedListMax=a.length;for(file of a)a=require("fs").readFileSync(normalizeDir(allPerkFolder)+"/"+file),a=JSON.parse(a),allPerks.push({filename:file,title:a.title})}
function drawEmptyScreen(){bC.setFontMonofonto18();bC.setColor(colourWhite);bC.drawString("No Perks selected, press wheel to configure.",2,100)}function drawEntry(a){drawEntryImage(a.img,a.xSize,a.ySize);drawEntryDesc(a.description)}function drawEntryImage(a,b,c){let d=Math.floor((perkImageXMaxSize-b)/2),f=Math.floor((perkImageYMaxSize-c)/2);bC.setColor(colourGray2);bC.drawImage({width:b,height:c,bpp:1,buffer:require("heatshrink").decompress(atob(a))},200+d,0+f)}
function drawEntryTitle(a,b,c){bC.setFontMonofonto18();c?bC.setColor(colourBlack):bC.setColor(colourWhite);bC.drawString(a,10,titleOffsetY*b+5)}function drawEntryTitleConfig(a,b,c,d,f){bC.setFontMonofonto18();a=f?a+" *":a;c?bC.setColor(colourBlack):bC.setColor(colourWhite);0==d?bC.drawString(a,10,titleOffsetY*b+5):bC.drawString(a,210,titleOffsetY*(b-configColumnDisplayMax)+5)}
...
(entrySelected=0),0<a&&lastReload-1==entrySelected?lastReload=null:0==entrySelected%entryListDisplayMax&&entrySelected!=lastReload&&(lastReload=null))}function handleKnob2(a){Pip.knob2Click(a);configMode=!1;screenSelected==perkSelectionScreen?(saveNewPerkSelection(),screenSelected=perkScreen):screenSelected+=a;entrySelected=0;lastReload=currentPerk=null;screenSelected>maxScreen?screenSelected=0:0>screenSelected&&(screenSelected=maxScreen)}
function handleTorch(){gracefulClose();torchButtonHandler()}function powerHandler(){gracefulClose()}function gracefulClose(){clearInterval(intervalId);Pip.removeListener("knob1",registeredKnob1Func);Pip.removeListener("knob2",handleKnob2);Pip.removeListener("torch",handleTorch);E.reboot()}let loadedListMax=0,entrySelected=0,screenSelected=0,pointsOfSelected=0,drawing=!1,configMode=!1,allPerks=[],enabledPerks=[],displayedPerks=[],currentPerk=null,lastReload=null,registeredKnob1Func=handleKnob1;
Pip.on("knob1",registeredKnob1Func);Pip.on("knob2",handleKnob2);Pip.on("torch",handleTorch);setWatch(powerHandler,BTN_POWER,{repeat:!1});draw();let intervalId=setInterval(()=>{checkMode();2==Pip.mode?draw():gracefulClose()},16);
