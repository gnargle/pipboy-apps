...
function gracefulClose(){clearInterval(intervalId);Pip.removeListener("knob1",registeredKnob1Func);Pip.removeListener("knob2",handleKnob2);Pip.removeListener("torch",handleTorch);E.reboot()}let loadedListMax=0,entrySelected=0,screenSelected=0,pointsOfSelected=0,drawing=!1,configMode=!1,allPerks=[],enabledPerks=[],displayedPerks=[],currentPerk=null,lastReload=null,registeredKnob1Func=handleKnob1;Pip.on("knob1",registeredKnob1Func);Pip.on("knob2",handleKnob2);Pip.on("torch",handleTorch);
