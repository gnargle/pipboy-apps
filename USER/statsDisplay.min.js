...
function buildScreen(a){try{var b=require("fs").readdirSync(a)}catch{require("fs").mkdir(a),b=require("fs").readdirSync(a)}loadedListMax=b.length;if(0==b.length)drawEmptyScreen();else{for(var c=Math.min(entryListDisplayMax,loadedListMax),d=0;entrySelected>=c;)d++,c=Math.min(entryListDisplayMax*(d+1),loadedListMax);for(i=d*entryListDisplayMax;i<c;i++)d=b[i],d=require("fs").readFileSync(a+d),d=JSON.parse(d),i==entrySelected&&(drawEntry(d),drawSelectedEntryOutline(i%entryListDisplayMax),configMode||
screenSelected==perkScreen||(pointsOfSelected=d.points)),drawEntryTitle(d.title,i%entryListDisplayMax,i==entrySelected),screenSelected!=perkScreen&&drawEntryPoints(d.points,i%entryListDisplayMax,i==entrySelected)}}
...
function saveFile(a){var b=require("fs").readdirSync(a);a+=b[entrySelected];b=require("fs").readFileSync(a);b=JSON.parse(b);b.points=pointsOfSelected;b=JSON.stringify(b);require("fs").writeFile(a,b)}function saveNewValue(){screenSelected==SPECIALScreen?saveFile(specialFolder):screenSelected==statScreen&&saveFile(skillsFolder)}function saveEnabledPerk(a){let b=require("fs").readFileSync(allPerkFolder+a);require("fs").writeFile(enabledPerkFolder+a,b)}
...
function handleKnob1(a){Pip.knob1Click(a);0==a?screenSelected==perkScreen?(entrySelected=0,screenSelected=perkSelectionScreen):screenSelected==perkSelectionScreen?togglePerkEnabled():(configMode=!configMode,Pip.removeListener("knob1",handleKnob1),Pip.on("knob1",handleKnob1Config),registeredKnob1Func=handleKnob1Config):(entrySelected-=a,0>entrySelected?entrySelected=loadedListMax-1:entrySelected>=loadedListMax&&(entrySelected=0));draw()}
function handleKnob2(a){Pip.knob2Click(a);configMode=!1;screenSelected==perkSelectionScreen?(saveNewPerkSelection(),screenSelected=perkScreen):screenSelected+=a;entrySelected=0;screenSelected>maxScreen?screenSelected=0:0>screenSelected&&(screenSelected=maxScreen);draw()}function handleTorch(){gracefulClose();torchButtonHandler()}function ourModeHandler(){checkMode();2!=Pip.mode&&gracefulClose()}function powerHandler(){gracefulClose()}
function gracefulClose(){clearInterval(modeCheck);clearInterval(intervalId);Pip.removeListener("knob1",registeredKnob1Func);Pip.removeListener("knob2",handleKnob2);Pip.removeListener("torch",handleTorch);showMainMenu()}let loadedListMax=0,entrySelected=0,screenSelected=0,pointsOfSelected=0,drawing=!1,configMode=!1,allPerks=[],enabledPerks=[],registeredKnob1Func=handleKnob1;Pip.on("knob1",registeredKnob1Func);Pip.on("knob2",handleKnob2);Pip.on("torch",handleTorch);setWatch(powerHandler,BTN_POWER,{repeat:!1});
log("reached predraw");draw();let modeCheck=setInterval(ourModeHandler,100),intervalId=setInterval(()=>{2==Pip.mode?draw():gracefulClose()},1E3);
