...
atob("BwYHCAgICAYHBggIBgcGCAgGCAgICAgICAgGBggICAgICAgICAcICAgICAgICAgICAgICAgICAgICAgICAUICAcICAgICAgICAcHCAYICAgICAgICAgICAgICAgGBwg="),65550)};function pathCombine(a,b){let c=a;a.endsWith("/")||(c+="/");return c+b}
function draw(){drawing||(drawing=!0,bC.clear(),screenSelected==perkScreen?buildScreen(enabledPerkFolder):screenSelected==statScreen?buildScreen(skillsFolder):screenSelected==SPECIALScreen?buildScreen(specialFolder):screenSelected==perkSelectionScreen&&buildPerkSelectionScreen(),bH.flip(),bF.flip(),bC.flip(),drawing=!1)}
function buildScreen(a){if(null==lastReload){displayedPerks=[];try{var b=require("fs").readdirSync(a)}catch{require("fs").mkdir(a),b=require("fs").readdirSync(a)}loadedListMax=b.length;if(0==b.length){drawEmptyScreen();return}let e=Math.min(entryListDisplayMax,loadedListMax);for(var c=0;entrySelected>=e;)c++,e=Math.min(entryListDisplayMax*(c+1),loadedListMax);for(i=c*entryListDisplayMax;i<e;i++){c=b[i];var d=require("fs").readFileSync(pathCombine(a,c));d=JSON.parse(d);screenSelected!=perkScreen?displayedPerks.push({title:d.title,
filename:c,entryNum:i,points:d.points}):displayedPerks.push({title:d.title,filename:c,entryNum:i})}lastReload=entrySelected}null==currentPerk&&(b=displayedPerks[entrySelected%6].filename,a=require("fs").readFileSync(pathCombine(a,b)),currentPerk=JSON.parse(a));for(a=0;a<displayedPerks.length;a++)b=displayedPerks[a],b.entryNum==entrySelected&&(drawEntry(currentPerk),drawSelectedEntryOutline(a),configMode||screenSelected==perkScreen||(pointsOfSelected=b.points)),drawEntryTitle(b.title,a,b.entryNum==
entrySelected),screenSelected!=perkScreen&&drawEntryPoints(b.points,a,b.entryNum==entrySelected)}
